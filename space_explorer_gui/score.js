...
// direction) → -100, at/above `hi` → +100, linear between. null if unknown.
function metricComponent(v, lo, hi, higherIsBetter) {
  if (v == null) return null;
  const t = Math.max(0, Math.min(1, (v - lo) / (hi - lo)));
  const good = higherIsBetter ? t : 1 - t; // 0..1, 1 = best
  return (good * 2 - 1) * 100; // -100..100
}
...
function naqComponent(s) {
  ...
  return a == null ? null : (a * 2 - 1) * 100; // -100..100
}
...
function seedScore(s) {
  ...
  for (const m of SCORE_METRICS) {
    const c = metricComponent(s[m.key], m.lo, m.hi, m.higherIsBetter);
    if (c == null) continue;
    sum += m.w * c;
    wsum += m.w;
  }
