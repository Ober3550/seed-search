...
#                      runner/bin/lua when it runs natively, else the `seedlua` image.
...
if [ -f "${REPO}/.env" ]; then
    set -a  # auto-export so subprocesses (ingame_dump.py) inherit vars
    source "${REPO}/.env"
    set +a
fi
...
DUMP="verifier/fixtures/universe-ingame-${SEED}.json"

# 1) Real-game dump.
python3 verifier/verify/ingame_dump.py "$SEED" -o "$DUMP"

# 2) Pick a Lua runner.
if [ -z "${SEED_LUA:-}" ]; then
  if runner/bin/lua -v >/dev/null 2>&1; then
    SEED_LUA="runner/bin/lua"
  else
    # -e SE_ENABLE_K2 forwards the host value so K2 mode reaches the container.
    SEED_LUA="docker run --rm --platform linux/amd64 -e SE_ENABLE_K2 -v ${REPO}:/w -w /w --entrypoint /w/runner/bin/lua-linux-x86_64 seedlua"
  fi
fi

# 3) Compare (compare.lua exits non-zero on any difference).
echo "=== comparing game dump vs seed finder for seed ${SEED} ==="
$SEED_LUA verifier/compare.lua "$DUMP" "$SEED"
